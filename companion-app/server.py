...
def get_media_properties(media_path):
  ...
  is_video = any(media_path.lower().endswith(ext) for ext in ['.mp4', '.mov', '.webm', '.mkv', '.avi'])
  ...
  command = [
    ...
    '-show_entries', show_entries, '-of', 'json', media_path
  ]
...
def process_video_task(job_id, source_path, background_path, settings):
  ...
  # Chroma key settings
  key_color = settings.get('keyColor', '0x00FF00')
  ...
  if not is_transparent:
    ...
    if background_path and background_path.lower().endswith(('.png', '.jpg', '.jpeg')):
  ...
  # Define input stream identifiers based on whether a background exists
  bg_stream = "[0:v]"
  fg_stream = "[0:v]" if is_transparent else "[1:v]"
  audio_stream = "0:a?" if is_transparent else "1:a?"

  # --- 4a. Resolution Scaling Logic ---
  job_info = JOBS.get(job_id, {})
  source_props = job_info.get('sourceProperties', {})
  background_props = job_info.get('backgroundProperties', {})
 
  final_res_str = None
  res_map = {'1080p': '1920:1080', '720p': '1280:720', '4k': '3840:2160'}

  if resolution_key in res_map:
    final_res_str = res_map[resolution_key]
  elif resolution_key == 'original_background' and background_props:
    final_res_str = f"{background_props['width']}:{background_props['height']}"
  elif resolution_key == 'original_source' and source_props:
    final_res_str = f"{source_props['width']}:{source_props['height']}"

  # If a final resolution was determined, build the scaling filters
  if final_res_str:
    scale_filter = f"scale={final_res_str}:force_original_aspect_ratio=decrease,pad={final_res_str}:-1:-1:color=black"
    if not is_transparent:
      filter_complex_parts.append(f"{bg_stream}{scale_filter}[bg_scaled]")
      bg_stream = "[bg_scaled]"
    filter_complex_parts.append(f"{fg_stream}{scale_filter}[fg_scaled]")
    fg_stream = "[fg_scaled]"

  # --- 4b. Chroma Keying Filter ---
  filter_complex_parts.append(f"{fg_stream}chromakey=color={key_color}:similarity={similarity}:blend={blend}:yuv=1[fg_keyed]")
 
  # --- 4c. Final Composition ---
  final_video_stream_name = "[outv]"
  if is_transparent:
    filter_complex_parts.append(f"[fg_keyed]format=yuva444p{final_video_stream_name}")
  else:
    overlay_options = "shortest=1" if duration is None else ""
    filter_complex_parts.append(f"{bg_stream}[fg_keyed]overlay=x=(W-w)/2:y=(H-h)/2:{overlay_options}{final_video_stream_name}")
  ...
  if output_format == 'prores':
    ...
    if is_transparent:
      command.extend(['-c:v', 'prores_ks', '-profile:v', '4', '-pix_fmt', 'yuva444p', '-c:a', 'pcm_s16le'])
    else:
      command.extend(['-c:v', 'prores_ks', '-profile:v', '3', '-c:a', 'pcm_s16le'])
  elif output_format == 'webm':
    output_extension = 'webm'
    command.extend(['-c:v', 'libvpx-vp9', '-b:v', '0', '-crf', '30', '-c:a', 'libopus'])
  elif output_format == 'gif':
    output_extension = 'gif'
    command.extend(['-vf', 'fps=15,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse'])
    ...
    command.extend(['-c:v', 'libx264', '-preset', 'medium', '-crf', '23', '-c:a', 'aac', '-b:a', '192k'])
  ...
  progress_duration = duration if duration and duration > 0 else JOBS[job_id].get('sourceProperties', {}).get('duration', 1)
  ...
  if process.returncode == 0:
    JOBS[job_id].update({'status': 'completed', 'progress': 100, 'outputPath': output_path})
  else:
    JOBS[job_id].update({'status': 'failed', 'error': f'FFmpeg failed with exit code {process.returncode}. Check logs for details.'})
...
def generate_preview_frame(job_id, settings):
  ...
  background_path = background_path_list[0] if background_path_list else None
 
  # 1. Extract a single frame from the source video
  preview_frame_path = os.path.join(job_dir, 'preview_base.png')
  duration = job.get('sourceProperties', {}).get('duration', 0)
  
  # Use specific timestamp if provided for scrubbing, otherwise use a default.
  timestamp_seconds = settings.get('timestamp')
  if timestamp_seconds is None:
    timestamp_seconds = max(1, duration * 0.1) if duration and duration > 1 else 0

  ts_hours = int(timestamp_seconds // 3600)
  ts_minutes = int((timestamp_seconds % 3600) // 60)
  ts_seconds = timestamp_seconds % 60
  timestamp_str = f"{ts_hours:02}:{ts_minutes:02}:{ts_seconds:06.3f}"

  extract_command = [
    'ffmpeg', '-y', '-ss', timestamp_str, '-i', source_path,
  ...
  # 2. Build and execute the final composition command
  output_image_path = os.path.join(job_dir, 'preview_composited.jpg')
  command = ['ffmpeg', '-y']
  filter_complex_parts = []
  ...
  similarity = settings.get('similarity', 0.2)
  blend = settings.get('blend', 0.1)
  chromakey_filter = f"chromakey=color={key_color}:similarity={similarity}:blend={blend}"

  if background_path:
    command.extend(['-i', background_path])
    command.extend(['-i', preview_frame_path])

    filter_complex_parts.append(f"[1:v]{chromakey_filter}[fg_keyed]")
    filter_complex_parts.append(f"[0:v]scale=1280:720:force_original_aspect_ratio=decrease,pad=1280:720:-1:-1:color=black[bg_scaled]")
    filter_complex_parts.append(f"[bg_scaled][fg_keyed]overlay=(W-w)/2:(H-h)/2[out]")
  else:
    command.extend(['-i', preview_frame_path])
   
    filter_complex_parts.append(f"color=c=gray:s=1280x720[c1]")
    filter_complex_parts.append(f"color=c=darkgray:s=1280x720[c2]")
    filter_complex_parts.append(f"[c1][c2]blend=all_expr='if(eq(mod(X/16,2),mod(Y/16,2)),A,B)'[checkerboard]")
    filter_complex_parts.append(f"[0:v]{chromakey_filter},scale=1280:720:force_original_aspect_ratio=decrease[fg_keyed]")
    filter_complex_parts.append(f"[checkerboard][fg_keyed]overlay=(W-w)/2:(H-h)/2[out]")
  ...
  if compose_proc.returncode == 0 and os.path.exists(output_image_path):
    ...
    print(f"Preview Error: Final composited image could not be created for job {job_id}.")
...
def preview_socket(ws):
  ...
  job_id = None
  # current_source_path is no longer needed here
  ...
  while True:
    try:
      ...
      if message.get('type') == 'init':
        ...
        # No need to set path, it's looked up dynamically
        ...
        if image_data:
          ...
          ws.send(json.dumps({"type": "error", "message": "Failed to generate preview."}))
...
def get_thumbnail():
  ...
  thumbnail_path = f"{temp_video_path}.jpg"

  command = [
    'ffmpeg', '-i', temp_video_path, '-vframes', '1', '-an', '-s', '400x225',
    '-ss', '00:00:01', thumbnail_path
  ]
...
def process_video_endpoint():
  ...
  if not is_transparent and 'background' not in request.files:
    return jsonify({"error": "A background file is required for non-transparent exports."}), 400
  ...
  job_data = {
    'id': job_id, 'status': 'uploaded', 'progress': 0,
    'settings': settings, 'sourceProperties': source_props
  }
  ...
  if background_file:
    ...
    if background_props:
      ...
      print(f"Warning: Could not read properties for background file: {background_file.filename}")
...
def export_video_endpoint(job_id):
  ...
  if not is_transparent:
    ...
    if not background_path_list:
      return jsonify({"error": "Background file missing on server for non-transparent export."}), 500
    background_path = background_path_list[0]

  JOBS[job_id].update({'status': 'queued', 'progress': 0, 'settings': settings})

  thread = threading.Thread(target=process_video_task, args=(job_id, source_path, background_path, settings))
  thread.start()
